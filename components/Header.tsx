  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        scrolled
          ? 'bg-gradient-to-r from-golf-green via-fairway-green to-golf-green shadow-2xl border-b-4 border-golf-gold'
          : 'bg-gradient-to-r from-golf-green/95 via-fairway-green/95 to-golf-green/95 backdrop-blur-md border-b-2 border-golf-gold/50'
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {navItems.map((item) => (
              ...
              >
                <span className="group-hover:animate-bounce-golf">{item.icon}</span>
                <span>{item.label}</span>
                <span className="absolute bottom-0 left-0 w-0 h-1 bg-golf-gold group-hover:w-full transition-all duration-300" />
              </button>
            ))}
          </div>
