...
function ItineraryImage({ src, alt, location }: { src: string; alt: string; location: string }) {
  ...
  if (imageError) {
    return (
      <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-fairway-green to-gray-700 flex items-center justify-center rounded-lg">
            ...
            >
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className="flex-shrink-0 w-12 h-12 bg-fairway-green text-white rounded-full flex items-center justify-center font-bold text-lg">
                    {day.day}
                  </div>
                ...
                <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-2">
                          {day.activities.map((activity, index) => (
                            <li key={index} className="flex items-start text-sm text-gray-600 dark:text-gray-400">
                              <svg className="w-4 h-4 mr-2 mt-0.5 flex-shrink-0 text-fairway-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {day.golfInfo && (
                        <div className="bg-fairway-green/20 dark:bg-fairway-green/20 p-3 rounded-lg">
                          <h4 className="font-semibold text-fairway-green dark:text-golf-gold mb-1">골프 정보</h4>
                          <p className="text-sm text-fairway-green dark:text-golf-gold">
                            {day.golfInfo.courseName} ({day.golfInfo.courseType})
                          </p>
                        </div>
                      )}
                    </div>
